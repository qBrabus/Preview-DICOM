...
import { DicomMetadataModal } from './DicomMetadataModal';
...
export const PatientDashboard: React.FC<PatientDashboardProps> = ({ onNavigate }) => {
  const { user, accessToken, logout, csrfToken, refresh } = useAuth();
  ...
  const handleExport = async () => {
    ...
    const recordIds = patientsToExport.map(p => p.recordId).filter((id): id is number => id !== undefined);

    if (recordIds.length === 0) {
      setError('Impossible d\'exporter : identifiants invalides.');
      return;
    }
    ...
    try {
      ...
      if (recordIds.length === 1) {
        // Single Export
        response = await fetch(`${API_BASE}/patients/${recordIds[0]}/export`, withAuth());
        filename = `Patient-${patientsToExport[0].id}.zip`;
      } else {
        // Bulk Export
        response = await fetch(`${API_BASE}/patients/export`, withAuth({
          method: 'POST',
          headers: {
            'Content-Type': 'application/json'
          },
          body: JSON.stringify({ patient_ids: recordIds })
        }));
        filename = `export_patients_${recordIds.length}.zip`;
      }
      ...
      console.error(err);
      setError('Impossible d\'exporter les patients.');
    }
  };
  ...
  return (
    <div className="flex flex-col h-screen bg-slate-50 overflow-hidden">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="h-8 w-8 bg-indigo-100 text-indigo-700 rounded-full flex items-center justify-center font-bold hover:bg-indigo-200 transition-colors"
                        ...
                        <div className="bg-slate-800 border-t border-slate-700 px-3 py-2 flex items-center gap-2 overflow-x-auto">
                          {selectedImages.map((image, index) => (
                            <button
                              ...
                              className={`px-3 py-2 rounded-md text-xs font-medium border transition-all whitespace-nowrap ${index === currentImageIndex
      ...
      {/* Metadata Modal */}
      {isMetadataModalOpen && selectedImages[currentImageIndex] && (
        <DicomMetadataModal
          instanceId={selectedImages[currentImageIndex].id}
